 ...
 * @description Lists all files in the working directory and its subdirectories. Returns a JSON object with the files paths relative to the working directory, exclude directories like node_modules, .git, dist, etc.
 * -eg. {
 *  "files": [
 *    "file1.txt",
 *    "src/file2.txt",
 ...
 * @description Reads the content of all files requested in the query parameter "files" and returns them as a JSON object
...
app.post("/create-files", async (req, res) => {
  ...
  if (!files || !Array.isArray(files)) {
    return res.status(400).json({
      message: 'Invalid request body. Expected a JSON object with an "files" property containing an array of file updates.',
      status: "error",
    })
  }
